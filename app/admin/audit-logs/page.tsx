...
const ACTION_GROUPS: Record<string, { label: string; actions: string[] }> = {
  ...
  inventory_changes: { label: "Inventory Changes", actions: ["ADJUST_INVENTORY"] },
  reconciliation: {
...
export default async function AuditLogsPage({ searchParams }: AuditLogsPageProps) {
  ...
  return (
    <main className="min-h-screen bg-app-bg p-4 md:p-8">
      <div className="mx-auto flex max-w-screen-2xl flex-col gap-6 animate-fade-in">
        <PageHeader
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card xl:sticky xl:top-6">
            ...
            <form method="get" className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <label className="block">
                  ...
                  <OmanDateInput name="startDate" defaultValue={resolvedSearchParams.startDate ?? ""} className="ui-input" />
                </label>
                <label className="block">
                  <span className="ui-label">Date To</span>
                  <OmanDateInput name="endDate" defaultValue={resolvedSearchParams.endDate ?? ""} className="ui-input" />
                </label>
                <label className="block">
                  <span className="ui-label">User</span>
                  <select name="userId" defaultValue={userId ?? ""} className="ui-input">
                ...
                <label className="block">
                  <span className="ui-label">Action Type</span>
                  <select name="actionGroup" defaultValue={actionGroup} className="ui-input">
                ...
                <label className="block">
                  <span className="ui-label">Target ID</span>
                  <input
                    type="text"
                    name="targetId"
              ...
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex items-center gap-2 text-sm font-black">
                  ...
                  <Link
                    ...
                    className={`rounded px-3 py-2 ${safePage > 1 ? "ui-btn ui-btn-primary" : "pointer-events-none bg-slate-200 text-slate-500"}`}
                  >
                    Prev
                  </Link>
                  <Link
                    ...
                    className={`rounded px-3 py-2 ${safePage < totalPages ? "ui-btn ui-btn-primary" : "pointer-events-none bg-slate-200 text-slate-500"}`}
                  >
                    Next
                  </Link>
                </div>
