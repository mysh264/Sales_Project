...
export default async function SalesmanDashboardPage() {
  ...
  const [unpaidInvoices, todaySales, cashCollectedToday, transfersCollectedToday, checksCollectedToday, todayPayments, latestInvoice] =
    await Promise.all([
      ...
      <section className="mx-auto flex max-w-md flex-col gap-4">
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {stats.map((stat) => (
            <article key={stat.label} className={`rounded-lg p-5 shadow-sm ${stat.tone}`}>
              <p className={`text-sm font-black uppercase tracking-wide ${stat.tone.includes("bg-ink") ? "text-slate-300" : "text-slate-500"}`}>
                {stat.label}
              </p>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <Link
            ...
            className="flex min-h-20 items-center justify-center rounded-lg bg-slate-950 px-5 text-center text-2xl font-black text-white shadow-lg active:scale-[0.99]"
          >
            My Sales History
          </Link>
        </div>
